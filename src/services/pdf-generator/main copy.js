...
function generateCompleteHtml(options) {
  ...
  if (pages && pages.length > 0) {
    ...
    // Add conditions as a separate page at the very end of all tests
...
function generatePageHtml(page, patientDetails, isLastPage, headerImg = null, footerImg = null) {
  ...
  const contentTop = `calc(${headerHeight} + 140px)`; // Header + patient details card (reduced from 190px)
  const contentHeight = `calc(${pageHeight} - ${headerHeight} - 140px - ${footerHeight})`;
...
function generatePathologyTable(chunk) {
  ...
  const rows = chunk.map(item => {
    ...
    // Try to get limits from parameter
    if (item.parameter?.lowerLimit !== undefined && item.parameter?.lowerLimit !== null) {
      lowerLimit = parseFloat(item.parameter.lowerLimit);
    }
    if (item.parameter?.upperLimit !== undefined && item.parameter?.upperLimit !== null) {
      upperLimit = parseFloat(item.parameter.upperLimit);
    }
    
    // Try to extract from ranges array
    if (item.parameter?.ranges?.[0]) {
      const rangeObj = item.parameter.ranges[0];
      if (rangeObj.lowerLimit !== undefined && rangeObj.lowerLimit !== null) {
        lowerLimit = parseFloat(rangeObj.lowerLimit);
      }
      if (rangeObj.upperLimit !== undefined && rangeObj.upperLimit !== null) {
        upperLimit = parseFloat(rangeObj.upperLimit);
      }
    }
    
    // Build display range text
    if (item.normalRangeText) {
      range = item.normalRangeText;
      
      // Parse normalRangeText to extract limits if we don't have them yet
      if (lowerLimit === null || upperLimit === null) {
        const rangeMatch = item.normalRangeText.match(/^(\d+\.?\d*)\s*-\s*(\d+\.?\d*)$/);
        if (rangeMatch) {
      ...
      // Parse referenceRange to extract limits if we don't have them yet
      if (lowerLimit === null || upperLimit === null) {
        const rangeMatch = item.parameter.ranges[0].referenceRange.match(/^(\d+\.?\d*)\s*-\s*(\d+\.?\d*)$/);
        if (rangeMatch) {
          lowerLimit = parseFloat(rangeMatch[1]);
          upperLimit = parseFloat(rangeMatch[2]);
        }
    ...
    if (!isNaN(numericValue) && (lowerLimit !== null || upperLimit !== null)) {
      ...
      if (isLow) {
        statusIndicator = `<span style="color: #dc2626; font-weight: bold; margin-left: 8px; font-size: 18px;">↓</span>`;
      } else if (isHigh) {
        statusIndicator = `<span style="color: #dc2626; font-weight: bold; margin-left: 8px; font-size: 18px;">↑</span>`;
      } else {
