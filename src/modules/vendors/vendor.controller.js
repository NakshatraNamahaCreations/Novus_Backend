...
export const deleteVendor = async (req, res) => {
  try {
    ...
    const existing = await prisma.vendor.findUnique({ where: { id: Number(id) } });
...
export const addVendorEarning = async (req, res) => {
  try {
    const { vendorId, title, desc, amount } = req.body;

    // Validate fields
    if (!vendorId || !title || !amount) {
      return res.status(400).json({
        error: "vendorId, title, and amount are required"
      });
    }
    ...
    const updatedVendor = await prisma.vendor.update({
      where: { id: Number(vendorId) },
      data: {
        earnings: vendor.earnings + Number(amount)
      }
    });
    ...
    return res.json({
      ...
      earning: historyRecord
    });

  } catch (error) {
    console.error("Error adding vendor earning:", error);
    res.status(500).json({
